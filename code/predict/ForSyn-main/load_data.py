...
cell_line_files=["A375", "A549", "BT20", "HCT116", "HS578T", "HT29",  "LOVO", "MCF7", "MDAMB231", "PC3", "RKO","SKBR3", "SKMEL28", "SW620","SW948", "VCAP"]


cell_line_path="data20210125/GP_977_final/"#插补前的GP
# cell_line_path="data20210125/GP_3908_final/"#插补后的GP
drugdrug_file="data20210125/DD.csv"
cell_line_feature="data20210125/16cell_zerodose_977cellname.csv"

def load_data(cell_line_name="all",score="S",dataid=3):
    ...
    for i in range(n_sample):
        ...
        if dataid==1:#只用GP
            sample=np.hstack((drugA_express,drugB_express))                                   #data3_1956
        if dataid==2:#8791=3908*2+977 用GP和细胞系
            sample=np.hstack((drugA_express,drugB_express,feature))
        if dataid==3:
            sample=np.hstack((drugA_finger,drugA_phychem,drugA_express,drugB_finger,drugB_phychem,drugB_express,feature))    #data3_1956
        # elif dataid==4:
