...
if os.path.exists('pyraformer'):
    shutil.rmtree('pyraformer')
...
pyraformer = Pyraformer(
    n_steps=num_doses,  # 每个样本有四个剂量  #L
    n_features=num_genes_per_dose,
    n_layers=2,#N 模型中的层数，决定了堆叠的注意力层的数量
    d_model=256,  # d_model= n_heads * d_k
    d_ffn=256,  # 128
    n_heads=4,
    window_size=[2,2],#C这是一个列表，表示在金字塔注意力机制中，每一层的下采样窗口大小。   S是列表的长度，列表的长度决定了金字塔的层级数
    inner_size=5,#A在金字塔注意力机制中，这个参数决定了每层注意力机制中邻接节点的数量。

    dropout=0.1,
    epochs=1000,
    saving_path="pyraformer",  # 设置保存路径
...
# 训练模型
pyraformer.fit(dataset)

# 填补缺失值
imputation = pyraformer.impute(dataset)
