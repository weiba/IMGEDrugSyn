...
data = pd.read_csv('D:/A_sig_cell_6h/final/6h_seq977.csv', index_col=0)  # 假设第一列为索引列，即基因名称
...
X = reshaped_data#因为这里是真实缺失的值，所有不需要再虚拟缺失了。
...
loaded_brits = BRITS(
    ...
    rnn_hidden_size=750,
    saving_path="brits",  # 之前保存的路径
    model_saving_strategy="best"
)
loaded_brits.load('D:/L1000/brits/BRITS_750.pypots')

# 使用加载的模型进行插补
imputation = loaded_brits.impute(dataset)
...
imputation_df.to_csv(os.path.join(saving_path, 'imputation_results_true.csv'))
