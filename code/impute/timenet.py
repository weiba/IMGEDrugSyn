...
if os.path.exists('timesnet'):
    shutil.rmtree('timesnet')
...
timesnet = TimesNet(
    ...
    n_layers=2,
    d_model=128,  # d_model= n_heads * d_k
    d_ffn=512,  # 128
    top_k=1,
    n_kernels=5,
    dropout=0.1,
    epochs=1000,
    saving_path="timesnet",  # 设置保存路径
...
# 训练模型
timesnet.fit(dataset)

# 填补缺失值
imputation = timesnet.impute(dataset)
